...
var braz = document.createElement('DIV'), txt = document.createTextNode('');
braz.appendChild(txt); braz.setAttribute("style", 'margin-bottom: ' + naveSize + 'px;'); braz.innerHTML = '<br><br>'; insertAfter(braz, nave);
var estilo = '#nave { width: 100%; display: block; left: 0; overflow: hidden; z-index: 3; } #nave a { text-decoration: none; z-index: 2; } @media screen and (min-width: 640px) { #nave { text-align: left; height: ' + naveSize + 'px; } #nave a { padding: 0 ' + (naveSize / 2) + 'px; font-size: ' + (naveSize / 2) + 'px; line-height: ' + naveSize + 'px; display: inline-block; } #nave span { padding: 0 ' + (naveSize / 2) + 'px; font-size: ' + (naveSize / 2) + 'px; line-height: ' + naveSize + 'px; display: inline-block; } } @media screen and (max-width: 640px) { #nave a { display: block; padding: ' + (naveSize / 2) + 'px 0; } } .n0s64t { position: absolute; } .stk007 { position: fixed; top: 0; }';
var css = document.createElement("STYLE"), style = document.createTextNode(estilo);
